...
Este módulo é fundamental para a personalidade do robô, permitindo que ele se comunique de forma clara e envolvente com os visitantes durante todo o tour.
...
<Mermaid chart="graph LR
    A[Perfil do robô] --> B[Personalidade do robô]
    B --> C[LLM API]" />

O módulo de personalidade do robô define as características comportamentais e o tom de voz do assistente virtual. Administradores podem configurar aspectos como formalidade, entusiasmo e estilo de comunicação através da interface de perfil do robô.

Esta configuração é utilizada pela LLM API para ajustar as respostas geradas, garantindo consistência na personalidade do robô ao longo de todas as interações. A personalidade pode ser ajustada conforme o público-alvo de cada tour, tornando a experiência mais adequada e envolvente.
...
Conforme descrito na camada de modelo, este serviço gerencia as configurações de personalidade que influenciam todas as respostas geradas pela LLM.
...
Interface que permite aos visitantes sinalizarem que estão prontos para avançar para o próximo checkpoint. Funciona como um sistema de votação ou confirmação coletiva, onde o grupo indica quando deseja prosseguir.

Este controle democratiza o ritmo do tour, permitindo que os visitantes passem mais ou menos tempo em cada ponto conforme seu interesse.
...
- Comunicação direta com visitantes
...
Interface para configuração da personalidade do robô:

- Tom de voz (formal, casual, entusiasta)
- Estilo de linguagem
- Tipo de saudação
- Comportamento em diferentes situações

As configurações são aplicadas imediatamente e afetam todas as interações subsequentes.
...
- **Personalidade do Robô**: Sistema de configuração de comportamento
